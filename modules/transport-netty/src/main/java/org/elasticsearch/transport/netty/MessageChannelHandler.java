...
import org.elasticsearch.transport.NettyUtils;
...
public class MessageChannelHandler extends SimpleChannelUpstreamHandler {
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
        ...
        try (ThreadContext.StoredContext tCtx = threadContext.stashContext()) {
            assert tCtx != null : "prevent compiler warning"; // NOT SURE WHAT THE RIGHT SUPPRESS IS!!!!!
            ...
            if (TransportStatus.isCompress(status) && hasMessageBytesToRead && buffer.readable()) {
                ...
                try {
                    compressor = CompressorFactory.compressor(NettyUtils.toBytesSequence(buffer));
            ...
            if (TransportStatus.isRequest(status)) {
                ...
                TransportResponseHandler<TransportResponse> handler = transportServiceAdapter.onResponseReceived(requestId);
    ...
    protected void handleResponse(Channel channel, StreamInput buffer, final TransportResponseHandler<TransportResponse> handler) {
    ...
    private void handlerResponseError(StreamInput buffer, final TransportResponseHandler<TransportResponse> handler) {
        Throwable error;
        try {
    ...
    private void handleException(final TransportResponseHandler<TransportResponse> handler, Throwable error) {
    ...
    protected String handleRequest(Channel channel, Marker marker, StreamInput buffer, long requestId, int messageLengthBytes,
        ...
        try {
            final RequestHandlerRegistry<TransportRequest> reg = transportServiceAdapter.getRequestHandler(action);
            if (reg == null) {
    ...
    class ResponseHandler implements Runnable {
        ...
        private final TransportResponseHandler<TransportResponse> handler;
        private final TransportResponse response;

        public ResponseHandler(TransportResponseHandler<TransportResponse> handler, TransportResponse response) {
    ...
    class RequestHandler extends AbstractRunnable {
        private final RequestHandlerRegistry<TransportRequest> reg;
        private final TransportRequest request;
        private final NettyTransportChannel transportChannel;

        public RequestHandler(RequestHandlerRegistry<TransportRequest> reg, TransportRequest request,
