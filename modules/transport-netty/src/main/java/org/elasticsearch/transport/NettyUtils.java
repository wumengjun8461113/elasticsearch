...
public class NettyUtils {
    ...
    public static final ChannelBuffer toChannelBuffer(BytesReference bytesReference) {
        ...
        bytesReference.forEach(consumer);
        return consumer.toChannelBuffer();
    }

    public static final BytesSequence toBytesSequence(ChannelBuffer channelBuffer) {
        return new BytesSequence() {
            final int offset = channelBuffer.readerIndex();
            @Override
            public byte get(int index) {
                return channelBuffer.getByte(offset + index);
            }
    ...
    private static class BufferConsumer implements BytesReference.PageConsumer {
        List<ChannelBuffer> buffers = new ArrayList<>();

        @Override
        public void consume(byte[] bytes, int index, int offset) {
            buffers.add(ChannelBuffers.wrappedBuffer(bytes, index, offset));
        }

        @Override
        public void consume(ByteBuffer byteBuffer) {
            buffers.add(ChannelBuffers.wrappedBuffer(byteBuffer));
        ...
        ChannelBuffer toChannelBuffer() {
            ...
            return ChannelBuffers.wrappedBuffer(NettyUtils.DEFAULT_GATHERING, buffers.toArray(new ChannelBuffer[buffers.size()]));
        }
    }
}
